...
pdf_directory = 'output/legal_notices/pdfs'
...
def download_pdf(pdf_url, filename, thread_name):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if FILE_STORAGE == 'aws':
                s3_file_location = upload_to_aws(local_file_path, f"legal_notices/{filename}")
            ...
            async def fetch_pdfs():
                async with async_playwright() as p:
                    ...
                    pdf_links = await page.eval_on_selector_all(
                        '.tx-jfmulticontent-pi1 a[href$=".pdf"]',
...
async def get_legal_files():
    print(f"File storage: {FILE_STORAGE}")
    async with async_playwright() as p:
        browser = await p.chromium.launch()
        
        # Start with the initial URL
        initial_url = 'https://kenyalaw.org/kl/index.php?id=12003'
